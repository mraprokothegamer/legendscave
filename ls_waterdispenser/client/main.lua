...
print('^2[ls_waterdispenser] v2.4.3 loaded^0')
...
local function attachCup()
    ...
    for m = 1, #models do
        ...
        if model and model ~= 0 and requestModel(model) then
            -- Local-only object. isNetwork=true makes CREATE_OBJECT call
            -- NETWORK_GET_NETWORK_ID_FROM_ENTITY; if no CNetObj exists the
            -- console prints: "no net object for entity (script id ...)".
            local cup = CreateObjectNoOffset(model, coords.x, coords.y, coords.z + 0.2, false, false, false)
            if (not cup or cup == 0 or not DoesEntityExist(cup)) then
                cup = CreateObject(model, coords.x, coords.y, coords.z + 0.2, false, false, false)
            end

            if cup and cup ~= 0 and DoesEntityExist(cup) then
                SetEntityAsMissionEntity(cup, true, true)
                if NetworkGetEntityIsNetworked(cup) then
                    NetworkUnregisterNetworkedEntity(cup)
                end
